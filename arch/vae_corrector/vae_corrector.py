...
class VAECorrector(L.LightningModule):
    """
    ...
    def training_step(self, batch: tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        loss = self.loss(x_gt, mu, logvar, z, x_hat_logits)
    ...
    def validation_step(self, batch: tuple[torch.Tensor, torch.Tensor]):
        ...
        # Compute loss using ground truth mask
        loss = self.loss(x_gt, mu, logvar, z, x_hat_logits, log_components=False)
    ...
    def test_step(self, batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]):
        ...
        Dice scores.
        """
        x_gt, x_pred, condition, ed = batch
        ...
        self.log_reconstruction_metrics(x_gt, x_pred, condition_label, phase_label)
        self.log_generation_metrics(x_gt)
        
        # TODO
        # self.x_buffer.append(x)
    
    def log_reconstruction_metrics(self, x_gt: torch.Tensor, x_pred: torch.Tensor, condition: str, phase: str):
        """
        ...
        Args:
            x (torch.Tensor): One-hot encoded input segmentations.
        """
        num_samples, _, _, _ = x_gt.shape
        
        _, _, _, x_hat_logits = self(x_pred, test=True)
        
        # Compute reconstruction loss
        recon_loss = self.reconstruction_loss(x_gt, x_hat_logits)
