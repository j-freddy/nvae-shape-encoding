...
class DecoderResidualCell(nn.Module):
    ...
    Implementation as described by Fig. 3a in the NVAE paper. InstanceNorm
    momentum is set to 0.05 in official NVAE implementation. Expand factor for
    depthwise convolution is set to either 6 (MobileNetV2) or 3. The latter
    reduces memory.
    """
    ...
    def __init__(self, num_channels: int, expand_factor: int=6):
        ...
        self.net = nn.Sequential(
            ...
            # IN + Swish
            nn.InstanceNorm2d(num_features=num_channels * expand_factor, eps=1e-5, momentum=0.05),
            nn.SiLU(),
